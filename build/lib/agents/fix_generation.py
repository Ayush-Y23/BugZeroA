...
def _build_file_patch_for_finding(
    *,
    ...
    """
    Build a minimal unified-diff style `FilePatch` that inserts a single
    investigation comment near the suspected error location.
    """
    span = finding.span
...
class FixGenerationAgent:
    ...
    The MVP implementation focuses on producing small diffs that:
    - Mark suspicious regions with an investigation comment; and
    - Capture rationale in the `ProposedFix.description`.

    This keeps changes safe and interpretable while leaving room for more
    advanced, automated repairs in later phases.
    """
